...
const connectedXKeys = devices.filter((device) => {
	return (
		device.vendorId === XKeys.vendorId &&
		device.usagePage === 12 &&
		device.usage === 1 &&
		(device.interface === 0 || device.interface === -1) &&
		device.productId !== badPID
	) // Make sure that it is consumer page and the interface-property is set to 0 or -1
})
...
if (connectedXKeys.length) {
	...
	console.log(
		'Interface= ' +
			connectedXKeys[0].interface +
			' UsagePage= ' +
			connectedXKeys[0].usagePage +
			' usage= ' +
			connectedXKeys[0].usage
	)
} else {
...
myXkeysPanel.on('down', (keyIndex, keyLocation, UID, PID, productName, timeStamp) => {
	console.log(
		'Key pressed:  Index: ' +
			...
			' Time Stamp: ' +
			timeStamp
	) // report the key index and the Unit ID UID
